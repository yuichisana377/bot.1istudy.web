    ...
    periodsHtml = basePeriods.map((p, i) => {
      ...
      if (periodHolidayOv) {
        ...
        return `<div class="period-row" onclick="showTTDetail('${dateStr}', ${periodNum})">
          ...
          <div class="period-subject" style="color:var(--text-tertiary)"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-3px;flex-shrink:0" aria-hidden="true"><circle cx="12" cy="12" r="8.3"/><path d="M6.6 6.6l10.8 10.8"/></svg> ${escapeAttr(phReason)}${phNote}</div>
...
function showTTDetail(dateStr, period) {
  ...
  if (holidayOv) {
    const note = holidayOv.note ? `（${escapeAttr(holidayOv.note)}）` : '';
    rows.push(`<div class="detail-item"><div class="dl-label">状態</div><div class="dl-value"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-3px;flex-shrink:0" aria-hidden="true"><circle cx="12" cy="12" r="8.3"/><path d="M6.6 6.6l10.8 10.8"/></svg> ${escapeAttr(holidayOv.reason) || '休み'}${note}</div></div>`);
  } else {
    ...
    if (items.length) {
      rows.push(`<div class="detail-item"><div class="dl-label">持ち物</div><div class="dl-value">${items.map(it => `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-3px;flex-shrink:0" aria-hidden="true"><path d="M16.3 6.7 8.9 14.1a3 3 0 0 0 4.2 4.2l7.6-7.6a5 5 0 0 0-7-7l-7.6 7.6a1.8 1.8 0 0 0 2.5 2.5l6.7-6.7"/></svg> ${escapeAttr(it)}`).join('　')}</div></div>`);
    }
